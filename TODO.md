...
- Settings → Supported Agents: WebLLM plus add-on-demand remote-agent rows (Anthropic, OpenAI, xAI — the three currently testable) with per-agent Commit/Remove, key fields auto-populated from Secrets on render, and boot-time purge of secrets/entries for any provider no longer supported. This is the Secrets UI, scoped per-agent instead of a generic named-key manager. **UI shell only** — no `backend.chat()` call or live model-list fetch wired yet; model dropdowns are placeholders.
...
|---|---|---|---|---|---|
| High | Medium | Backends | Backend interface + Remote | Local WebLLM (1.5B–7B) is free/private but too weak to carry Sight-driven reasoning or scaffolding discipline. Everything below this row needs a genuinely capable model to even be testable — the wall we hit this session. | `backend.chat(messages, {tools, stream})` interface; the UI shell is done (Settings → Supported Agents — see Already built), scoped to Anthropic/OpenAI/xAI, the three currently testable. Still needed: the actual fetch call per provider (auth header shape + endpoint verified via live CORS checks this session), and populating each model dropdown from a live `/models` request on Commit instead of the current placeholder. Mistral, Meta Llama, and Gemini are deferred (not yet testable) but already confirmed CORS-reachable — see the Backlog row. |
...
3. **Secrets/remote — partly resolved.** Storage is the existing encrypted-at-rest layer (device-bound); the Secrets UI question is settled — per-agent key fields in Settings → Supported Agents (see Already built), not a generic named-key manager. Still open: is device-bound persistence sufficient, or do we also want a session-only "don't save" mode?
...
|---|---|---|---|---|
| Low | Low | Additional remote agents (Mistral, Meta Llama, Gemini) | The Supported Agents shell is scoped to Anthropic/OpenAI/xAI — the three the author can currently test with a real key. Mistral, Meta Llama API, and Google Gemini are real, viable providers (all confirmed CORS-reachable for both chat and `/models`-list endpoints this session) but untested, so left out for now rather than shipped unverified. | Add each to `AGENT_PROVIDERS` once testable. Gemini needs a different code path either way (`x-goog-api-key` header, model-in-URL-path request shape, different response JSON) — not a drop-in of the other four's OpenAI-compatible shape. |
